...
) do for /f "tokens=1-5 delims=:" %%A in ("%%#") do if /i %osver% GEQ %%A if /i [%osbuild%]==[%%B] if /i [%ostype%]==[%%C] if /i [%osedition%]==[%%D] set "key=%%E"
echo:
if not defined key (
    %_eline%
...
if defined _slicexist (
    ...
    for %%# in (
        ...
        10.0:17763:5
        10.0:20348:6
    ) do for /f "tokens=1-3 delims=:" %%A in ("%%#") do if /i !osver! GEQ %%A if /i [!osbuild!]==[%%B] set "_expctslicveroffst=%%C"
...
for %%# in (
    ...
    10.0:17763:2.5
    10.0:20348:2.6
) do for /f "tokens=1-3 delims=:" %%A in ("%%#") do if /i %osver% GEQ %%A if /i [%osbuild%]==[%%B] set "_slicvertobeinstalled=%%C"
